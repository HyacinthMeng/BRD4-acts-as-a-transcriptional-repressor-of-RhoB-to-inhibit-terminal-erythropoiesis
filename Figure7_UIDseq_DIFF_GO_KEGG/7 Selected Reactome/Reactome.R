...
GO = read.table("R1_Reactome.txt",header=TRUE,sep="\t",quote = "")  #读取第1部分enrichGO分析输出的文件eG。

View(GO)
GO = read.table("R1_UP_id.txt",header=TRUE,sep="\t",quote = "")  #读取第1部分enrichGO分析输出的文件eG。
View(GO)
...
p1 <- ggplot(GO,aes(Count,reorder(Description,Count))) +    #reorder(Description,Count)
 ...
 scale_color_gradient(low = "#FF9EA3",high="#E82338",  #  "#FFE0E1"  #low = 'blue',high="#d90424"
                     limits=c(3,10)  # 自定义范围   limits=c(1,12)
...
ggsave("R——UP_GO_1201_bubble_pvalue-2.pdf", width =6 , height = 5)

ggsave("R——UP_Reactome_1201_bubble_pvalue.pdf", width =6.5 , height = 5)
