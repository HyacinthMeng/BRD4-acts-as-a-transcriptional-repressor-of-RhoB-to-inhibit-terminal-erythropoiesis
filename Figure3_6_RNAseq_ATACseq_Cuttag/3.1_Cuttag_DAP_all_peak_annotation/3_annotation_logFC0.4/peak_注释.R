...
##== 1 读取数据
peak <- readPeakFile("edgeR_logFC0.4.bed")
...
# 1. 创建画布
png(filename = "PeakProf.png",width = 5,height = 5,units = "px",bg = "white",res = 72)             
...
#####
png(filename = "reactome.png", width = 480,height = 550,units = "px",bg = "white",res = 72)             
...
####
pdf("reactome.pdf",width = 9,height = 10)              # 分辨率
dotplot(pathway1,x = "GeneRatio",color = "p.adjust",showCategory = 10,size = NULL,
        split = NULL,font.size = 12,title = "",orderBy = "x",label_format = 30,
)
dev.off()
...
dotplot(pathway1)
write.table(pathway1,'reactome_pathway.xls',sep = '\t', quote = FALSE, row.names = FALSE)
